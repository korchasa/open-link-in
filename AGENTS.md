...
- `deno task prod` — compile release, assemble `SmartLinksOpener.app`, sign (Hardened Runtime, ad-hoc) and install it as **`/Applications/Reroute Dev.app`** (`scripts/install.ts`: bundle id `dev.korchasa.SmartLinksOpener.dev`, name "Reroute Dev", version suffixed with the commit), relaunching it. The dev copy sits next to the App Store build, keeps its own rules (defaults domain `…SmartLinksOpener.dev`) and, being a browser, appears in the store build's browser list (hide it there). The repo-path bundle keeps the store id and is deliberately NOT registered with LaunchServices.
...
> ⚠️ **Local verification — bundle-id collision.** the bundle assembled in the repo (`SmartLinksOpener.app`) and the `dist` bundle carry the store id `dev.korchasa.SmartLinksOpener`, the same as the App Store copy in `/Applications`. Never register or launch them by that id — test the **Reroute Dev** copy instead (`open -a "Reroute Dev" <url>` sends the link to it). If a stray registration exists, `lsregister -u <path>` it; `pgrep -fl SmartLinksOpener` should list the store copy and the dev copy only.
